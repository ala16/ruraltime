...
export function Apoiadores() {
  ...
  const apoiadores = [
    ...
    { nome: "Aromas e Sabores", logo: aromasSaboresLogo, alt: "Logo Aromas e Sabores" },
    { nome: "Rancho do Ferreiro", logo: ranchoFerreiroLogo, alt: "Logo Rancho do Ferreiro" },
    { nome: "São Ignácio", logo: saoIgnacioLogo, alt: "Logo São Ignácio" },
    { nome: "Candida Baptista", logo: candidaBaptistaLogo, alt: "Logo Candida Baptista" },
    ...
    { nome: "Wine Trade Fair", logo: wineTradeFairLogo, alt: "Logo Wine Trade Fair" },
  ];

  const duplicated = [...apoiadores, ...apoiadores];

  return (
    <section id="apoiadores" className="py-4 bg-muted/30 overflow-hidden" aria-label={`${t('supporters.our')} ${t('supporters.label')}`}>
      <div className="max-w-6xl mx-auto px-4">
        <h2 className="text-center text-sm text-muted-foreground mb-3">
          {t('supporters.our')} <strong className="text-foreground">{apoiadores.length} {t('supporters.label')}</strong> {t('supporters.suffix')}
        </h2>
